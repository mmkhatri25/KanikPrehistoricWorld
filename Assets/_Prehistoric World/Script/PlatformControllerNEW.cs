...
public class PlatformControllerNEW : RaycastController, IListener {
	...
	public enum MovingType { AutoMove, PlayerStandOn, PlayerFirstStandOn}
	[Header("--------------------")]
	public Vector2 forceMovingDirection = Vector2.zero;	//Vector != zero: force moving this direction
	public MovingType movingType;
	public enum LoopType { Reverse, Cyclic, BackToStartPoint, NoLoop}
	
	public LoopType loopType;
	public bool fallingWhenNoLoop = false;
...
//	public bool isStickPlayer = false;
   ...
   [ReadOnly] public bool isStop = true;
	...
	public void InitNew(Vector2 forceDirection, float newSpeed)
    {
		forceMovingDirection = forceDirection;
		speed = newSpeed;
    }
    ...
    void LateUpdate () {
		...
		if (isFallingDown)
		{
			_fallingSpeed = Mathf.Lerp(_fallingSpeed, fallingSpeed, 3 * Time.deltaTime);
			velocity =  Vector2.down * _fallingSpeed * Time.deltaTime;
		}
		else if (forceMovingDirection!=Vector2.zero)
        {
			velocity = forceMovingDirection * speed * Time.deltaTime;
		}
		else
...
//		} else {
        ...
        if ((movingType == MovingType.AutoMove || isStanding) && !isStop)
        {
            MovePassengers(true);
            transform.Translate(velocity, Space.World);
            MovePassengers(false);
        }
        else if (((movingType == MovingType.PlayerStandOn || movingType == MovingType.PlayerFirstStandOn) && !isStanding) || isStop)
        {
	...
	Vector3 CalculatePlatformMovement() {
		...
		if (percentBetweenWaypoints >= 1) {
			...
			if (loopType == LoopType.NoLoop && (fromWaypointIndex >= globalWaypoints.Length - 1)) {
				isFinishedWays = true;
				if (workingFX && loopType == LoopType.NoLoop && fallingWhenNoLoop)
					workingFX.SetActive(false);

				if (lineRen)
					lineRen.enabled = false;

				if (fallingWhenNoLoop)
				{
					isFallingDown = true;
					...
					enabled = false;
			}else if (loopType == LoopType.BackToStartPoint)
            {
	...
	bool CalculatePassengerMovement(Vector3 velocity) {
		...
		if (movingType == MovingType.PlayerFirstStandOn && isStanding)
		{
			movingType = MovingType.AutoMove;       //switch to auto move when detect player standing on
		}
	...
	void OnDrawGizmos() {
		...
		if (forceMovingDirection != Vector2.zero)
		{
			lineRen = GetComponent<LineRenderer>();
			if (lineRen)
...
//				Gizmos.DrawLine(globalWaypointPos - Vector3.left * size, globalWaypointPos + Vector3.left * size);
				if ((i + 1 >= localWaypoints.Count) && loopType == LoopType.NoLoop && loopType == LoopType.Reverse)
				...
				if (i + 1 >= localWaypoints.Count) {
					if (loopType == LoopType.Cyclic && loopType != LoopType.NoLoop) {
