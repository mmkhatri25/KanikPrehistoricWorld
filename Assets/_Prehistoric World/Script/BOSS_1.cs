...
public class BOSS_1 : BossManager, ICanTakeDamage, IListener
{
    ...
    public float forceKnockBack = 20;
    [Header("EARTH QUAKE")]
    ...
    void Start()
    {
        ...
        if (FlyingAttackDurationFX)
            FlyingAttackDurationFX.SetActive(false);
    ...
    void Update()
    {
        ...
        if (!isKnockingBack && !isMeleeAttacking && (Time.time > (ME_LastAttack + MA_rate)) && (punchType == PUNCHTYPE.Auto && !isDoingOtherAttacking()) || (punchType == PUNCHTYPE.OnlySpeedAttack && isAttackSpeed))
        {
            if (Physics2D.CircleCast(MeleePoint.position, meleeDamageZone, Vector2.zero, 0, 1 << LayerMask.NameToLayer("Player")))
            {
                MeleeAttack();
    ...
    [ReadOnly] public bool isKnockingBack = false;
    ...
    public void TakeDamage(float damage, Vector2 force, GameObject instigator, Vector3 hitPoint)
    {
        ...
        if (isDead)
        {
            ...
            if (isFallingObjectAttack)
                tempFallingBullet.RemoveAllBullets();
            ...
            if (deadBehavior == DEAD_BEHAIVIOR.FinishLevel && (bossType == BOSSTYPE.BOSS))
            {
        ...
        {
            if (!isMeleeAttacking && !isDoingOtherAttacking())
                anim.SetTrigger("hit");

            StartCoroutine(BlinkEffecrCo());		//begin the blink effect
            SoundManager.PlaySfx(hurtSound, 0.7f);

            if (!isFlyingAttack && !isFallingObjectAttack)
            {
                velocity = new Vector2(Mathf.Abs(forceKnockBack) * (transform.position.x > GameManager.Instance.Player.transform.position.x ? 1 : -1), 0);

                StartCoroutine(KnockingBackCo());
            }
        }
    ...
    void MeleeAttack()
    {
        ...
        if (comboFX)
        {
            Instantiate(comboFX, MeleePoint.position, Quaternion.identity).transform.localScale = new Vector3((reverseFX ? -1 : 1) * (isFacingRight() ? 1 : -1), 1, 1);
        }
    ...
    {
        var hit = Physics2D.CircleCast(MeleePoint.position, meleeDamageZone, Vector2.zero, 0, 1 << LayerMask.NameToLayer("Player"));
        if (hit)
    ...
    [ReadOnly] public bool isAttackSpeed = false;
    ...
    FallingProjectileBullet tempFallingBullet;
    ...
    IEnumerator FallingObjectAttackCo()
    {
        ...
        tempFallingBullet = Instantiate(fallingAttackBullet, transform.position + Vector3.up * spawnBulletFX /** (GameManager.Instance.Player.inverseGravity ? -1 : 1)*/, Quaternion.identity);
        tempFallingBullet.Init(numberBulletFA, delayFA, speedFA, spaceFA);
    ...
    public GameObject FlyingAttackDurationFX;
    ...
    IEnumerator FlyingAttackCo(bool throwStone = false, bool throwSpreadBullet = false)
    {
        ...
        else
        {
            ...
            if (GameManager.Instance.Player.isPlaying)
            {
                ...
                LookAtPlayer();
                var hit = Physics2D.Raycast(centerPoint.position, (GameManager.Instance.Player.controller.boxcollider.bounds.center - centerPoint.position), 100, 1 << LayerMask.NameToLayer("Player"));
                if (hit)
                {
                    targetFlyingAttack = hit.point;
                }
                else
                    targetFlyingAttack = GameManager.Instance.Player.controller.boxcollider.bounds.center;

                SoundManager.PlaySfx(airAttackSound);
                anim.SetBool("isFlyingAttack", true);
                if (FlyingAttackDurationFX)
                    FlyingAttackDurationFX.SetActive(true);
                while (Vector2.Distance(transform.position, targetFlyingAttack) > 0.5f)
                {
                ...
                var hitGround = Physics2D.CircleCast(centerPoint.position, 1, Vector2.zero, 0, 1 << LayerMask.NameToLayer("Player"));
                if (hitGround)
    ...
    IEnumerator TORNADOAttackCo()
    {
        ...
        {
            Instantiate(TA_Tornado, transform.position + Vector3.down * 0.2f, Quaternion.identity).Init(TA_twoDirection, TA_damagePerBullet, TA_bulletSpeed);
