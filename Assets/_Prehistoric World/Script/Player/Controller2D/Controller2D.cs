...
{
    [Header("JUMP THROUGH 'through' Platform")]
    public bool canJumpUp = true;
    public bool canJumpDown = true;
    public bool forceCollideWithThroughPlatform = false;        //alway collide with the 'Through' tag Platform even in the air
    ...
    void HorizontalCollisions(ref Vector3 velocity)
    {
        ...
        for (int i = 0; i < horizontalRayCount; i++)
        {
            ...
            RaycastHit2D[] hits = Physics2D.RaycastAll(rayOrigin, Vector2.right * directionX, rayLength, useSperateHorizontal ? collisionMasHorizontal : collisionMask);
            ...
            foreach (var hit in hits)
            {
                if (hit && hit.collider.gameObject != gameObject)
                {
                    ...
                    if (hit.distance == 0 || (!forceCollideWithThroughPlatform && hit.collider.gameObject.CompareTag("Through") && (!isBelowOld || (isBelowOld && i != 0))))
                    {
                        continue;
                    }
                    ...
                    if (!collisions.climbingSlope || slopeAngle > maxClimbAngle)
                    {
                        ...
                        collisions.hitLeft = directionX == -1 ? hit.collider.gameObject : null;
                        collisions.hitRight = directionX == 1 ? hit.collider.gameObject : null;
        ...
        for (int i = 0; i < verticalRayCount; i++)
        {
            ...
            foreach (var hit in hits)
            {
                if (hit && hit.collider.gameObject != gameObject)
                {
                    ...
                    {
                        if (canJumpUp && (directionY == 1 || hit.distance == 0))
                        {
                            continue;
                        }
                        ...
                        if (canJumpDown && playerInput.y == -1)
                        {
    ...
    public struct CollisionInfo
    {
        ...
        public GameObject hitBelowObj, hitAboveObj, hitLeft, hitRight;
