        ...
        if (isMoving)
        {
            ...
            if (hits.Length > 0)
            {
                foreach (var hit in hits)
                {
                    ...
                    {
                        Instantiate(destroyFX, transform.position + Vector3.down, Quaternion.identity);
            ...
            {
                hits = Physics2D.CircleCastAll(transform.position, 0.5f, Vector2.zero, 0, 1 << LayerMask.NameToLayer("Platform"));
                if (hits.Length > 0)
                {
                    foreach (var hit in hits)
                    {
