...
public class UltiHelper {
    ...
    public static LayerMask AddLayerMask(LayerMask layer, string layerName)
    {
        return layer & ~(1 << LayerMask.NameToLayer(layerName));
    }

    public static LayerMask SubtractLayerMask(LayerMask layer, string layerName)
    {
        return layer | (1 << LayerMask.NameToLayer(layerName));
    }

    public static LayerMask AddLayerMask(LayerMask layer, int layerID)
    {
        return layer & ~(1 << layerID);
    }

    public static LayerMask SubtractLayerMask(LayerMask layer, int layerID)
    {
        return layer | (1 << layerID);
    }
}
