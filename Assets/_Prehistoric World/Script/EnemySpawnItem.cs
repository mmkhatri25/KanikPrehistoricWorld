...
public class EnemySpawnItem : MonoBehaviour {
    ...
    public GameObject forceSpawnObj;        //static object
    
    [Header("SET UP")]
    public AudioClip sound;
    public Vector2 localOffset = new Vector2(0, 1);
    public Vector2 spawnForce = new Vector2(2, 3);
    ...
    public void SpawnItem()
    {
            ...
            for (int i = 0; i < spawnNumber; i++)
            {
                if (randomChanceItem.Length > 0)
                {
                    if (Random.Range(0f, 1f) < chanceSpawn)
                    {
                        ...
                        {
                            Instantiate(randomChanceItem[pickObj], transform.position + (Vector3)localOffset, Quaternion.identity).Init(true, new Vector2(Random.Range(-1f, 1f) * spawnForce.x, spawnForce.y));
                        }
                    }
                }
            }
        ...
        if (useForceSpawnItem)
        {
            ...
            if (forceSpawnRandomItem.Length > 0)
            {
                ...
                {
                    Instantiate(forceSpawnRandomItem[pickObj], transform.position + (Vector3)localOffset, Quaternion.identity).Init(true, new Vector2((Random.Range(-1f, 1f)) * spawnForce.x, spawnForce.y));
                }
            }
        }
